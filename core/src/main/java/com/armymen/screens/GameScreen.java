...
public class GameScreen implements Screen {
    ...
    private void update(float delta) {
        ...
        passiveShoot(playerSoldiers, enemyUnits);
        passiveShoot(enemySoldiers,  playerUnits);
    ...
    // --- Disparo pasivo para unidades móviles ---
    private void passiveShoot(Array<SoldierBasic> shooters, Array<Unit> victimsSide) {
        ...
        for (int i = 0; i < shooters.size; i++) {
            ...
            Unit t = findNearestInRange(s, victimsSide, s.getAttackRange());
            if (t != null && s.canShoot()) {
                Projectile proj = new Projectile(s.getPosition(), t.getPosition(),
                    shooters == playerSoldiers);
    ...
    private void updateProduction(float dt) {
        for (int i = 0; i < bdata.size; i++) {
            ...
            if (d.current == null && d.queue.size > 0) d.current = d.queue.removeIndex(0);
    ...
    private void updateProjectiles(float dt) {
        ...
        for (int i = 0; i < projectiles.size(); ) {
            ...
            // choque con FORTINES del jugador (si el disparo es enemigo)
    ...
    private void handleInput() {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.S)) {
            ...
            if (hq != null && !hq.b.isUnderConstruction() && plastic >= COST_SOLDIER) {
                plastic -= COST_SOLDIER;
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.R)) {
            BData hq = firstOfType(BType.HQ);
            if (hq != null && !hq.b.isUnderConstruction() && plastic >= COST_SWEEPER) {
                plastic -= COST_SWEEPER;
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.V)) {
            ...
            if (gar != null && !gar.b.isUnderConstruction() && plastic >= COST_TRUCK) {
                plastic -= COST_TRUCK;
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.T)) {
            BData gar = firstOfType(BType.GARAGE);
            if (gar != null && !gar.b.isUnderConstruction() && plastic >= COST_TANK) {
                plastic -= COST_TANK;
    ...
    // Construcción con bulldozer (diferida)
    private void tryBuildAt(Vector2 dest) {
        // Evitar construir sobre otro edificio
        for (int i = 0; i < buildings.size; i++) {
            if (buildings.get(i).getBounds().contains(dest)) return;
        }

        // FORTÍN: ahora también requiere ir con el bulldozer y tarda 3s
        ...
        // Edificios con obra (HQ/Storage/Garage): programar y mover bulldozer
    ...
    private void updateBulldozerBuild() {
        ...
        if (bulldozer.getPosition().dst2(pendingBuild.pos) <= BUILD_START_RADIUS * BUILD_START_RADIUS) {
            ...
            // HQ / STORAGE / GARAGE: edificio clásico con Building
    ...
    private void checkEndConditions() {
        boolean playerHasBuildings = (playerFortins.size > 0) || (bdata.size > 0);
        boolean enemyHasForces = (enemyUnits.size > 0); // cuando tengan edificios enemigos, extender acá

        if (!playerHasBuildings) gameState = GameState.DEFEAT;
        else if (!enemyHasForces) gameState = GameState.VICTORY;
    }
    ...
    private void drawGameOverOverlay() {
        ...
        shape.rect(x, y, camera.viewportWidth, camera.viewportHeight);
        shape.end();
        ...
        String title = (gameState == GameState.VICTORY) ? "VICTORIA" : "DERROTA";
        font.setColor(Color.WHITE);
        font.draw(batch, title, camera.position.x, camera.position.y + 24, 0, Align.center, false);

        font.getData().setScale(1.0f);
        font.setColor(new Color(1f,1f,1f,0.9f));
        font.draw(batch, "ENTER para reiniciar", camera.position.x, camera.position.y - 14, 0, Align.center, false);
